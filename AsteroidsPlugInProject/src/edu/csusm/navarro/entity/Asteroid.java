...
public class Asteroid extends Entity {
	 ...
	 */
	public Asteroid(Random random, IVectorFactory vectorFactory) {
		super(calculatePosition(random, vectorFactory), calculateVelocity(random, vectorFactory), AsteroidSize.Large.radius, AsteroidSize.Large.killValue);
	 ...
	 * @param random The Random instance.
	 */
	public Asteroid(Asteroid parent, AsteroidSize size, Random random, IVectorFactory vectorFactory) {
		super(vectorFactory.createVector(parent.position), calculateVelocity(random, vectorFactory), size.radius, size.killValue);
		this.vectorFactory = vectorFactory;
		this.rotationSpeed = MIN_ROTATION + (random.nextDouble() * ROTATION_VARIANCE);
		this.size = size;
	 ...
	 */
	private static IVector calculatePosition(Random random, IVectorFactory vectorFactory) {
	    // Create a vector at the center of the world
	    IVector center = vectorFactory.createVector(WorldPanel.WORLD_SIZE / 2.0, WorldPanel.WORLD_SIZE / 2.0);

	    // Create a random offset vector and scale it
	    IVector offset = vectorFactory.createVector(random.nextDouble() * Math.PI * 2)
	                                  .scale(MIN_DISTANCE + random.nextDouble() * DISTANCE_VARIANCE);

	    // Return the sum of center and offset
	 ...
	 * @return The velocity.
	 */
	private static IVector calculateVelocity(Random random, IVectorFactory vectorFactory) {
		  // Create a random direction vector and scale it by a random speed
	    return vectorFactory.createVector(random.nextDouble() * Math.PI * 2)
	                        .scale(MIN_VELOCITY + random.nextDouble() * VELOCITY_VARIANCE);
	}
	...
	public void handleCollision(Game game, Entity other) {
		...
		if(other.getClass() != Asteroid.class) {
			...
			if(size != AsteroidSize.Small) {
				...
				for(int i = 0; i < 2; i++) {
					game.registerEntity(new Asteroid(this, spawnSize, game.getRandom(), vectorFactory));
				}
			}
