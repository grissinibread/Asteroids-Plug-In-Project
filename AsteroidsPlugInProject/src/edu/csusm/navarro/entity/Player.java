...
public class Player extends Entity {
	 ...
	 */
	public Player(IVectorFactory vectorFactory) {
		super(
				vectorFactory.createVector(WorldPanel.WORLD_SIZE / 2.0, WorldPanel.WORLD_SIZE / 2.0),
				vectorFactory.createVector(0.0, 0.0), 
				10.0, 
				0
		);
		
		Player.vectorFactory = vectorFactory;
	...
	public void update(Game game) {
		...
		if(thrustPressed) {
			 ...
			 */
			velocity.add(vectorFactory.createVector(rotation).scale(THRUST_MAGNITUDE));
		...
		if(firingEnabled && firePressed && fireCooldown <= 0 && overheatCooldown <= 0) {
			...
			if(bullets.size() < MAX_BULLETS) {
				...
				Bullet bullet = new Bullet(this, rotation, vectorFactory);
				bullets.add(bullet);
