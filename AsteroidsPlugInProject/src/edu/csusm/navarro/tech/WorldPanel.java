	...
	public void paintComponent(Graphics g) {
		...
		while(iter.hasNext()) {
			...
			if(entity != game.getPlayer() || game.canDrawPlayer()) {
				IVector pos = entity.getPosition(); //Get the position of the entity.
				
				//Draw the entity at it's actual position, and reset the transformation.
				drawEntity(g2d, entity, pos.getX(), pos.getY());
				...
				double x = (pos.getX() < radius) ? pos.getX() + WORLD_SIZE
						: (pos.getX() > WORLD_SIZE - radius) ? pos.getX() - WORLD_SIZE : pos.getX();
				double y = (pos.getY() < radius) ? pos.getY() + WORLD_SIZE
						: (pos.getY() > WORLD_SIZE - radius) ? pos.getY() - WORLD_SIZE : pos.getY();
