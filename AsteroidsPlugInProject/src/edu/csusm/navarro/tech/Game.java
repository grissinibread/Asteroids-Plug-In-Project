package edu.csusm.navarro.tech;
...
import edu.csusm.navarro.entity.Asteroid;
import edu.csusm.navarro.entity.Entity;
import edu.csusm.navarro.entity.Player;
import edu.csusm.navarro.vector.IVectorFactory;
import edu.csusm.navarro.clock.IClock;
...
public class Game extends JFrame {
	...
	private final IClockFactory clockFactory;
	
	private final IVectorFactory vectorFactory;
	
	public Game(IClockFactory clockFactory, IVectorFactory vectorFactory) {
        super("Asteroids"); 
        
        this.clockFactory = clockFactory; 
        this.vectorFactory = vectorFactory;
        
        // 3. Delegate the rest of the setup work.
	...
	public void startGame() {
		...
		this.player = new Player(vectorFactory);
		
		//Set the variables to their default values.
		resetGame();
		
		//Create the logic timer and enter the game loop.
		System.out.println("ClockFactory: " + clockFactory);
		this.logicTimer = clockFactory.createClock(FRAMES_PER_SECOND);
		while(true) {
	...
	private void updateGame() {
		...
		if(!isGameOver && areEnemiesDead()) {
			...
			for(int i = 0; i < level + 2; i++) {
				registerEntity(new Asteroid(random, vectorFactory));
			}
		}
